...
export default function AdminSettingsTab({ admin, t }: AdminSettingsTabProps): React.ReactElement {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100">
          <h2 className="font-semibold text-slate-900">{t('admin.travelpayoutsTitle')}</h2>
          <p className="text-xs text-slate-400 mt-1">{t('admin.travelpayoutsSubtitle')}</p>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('admin.travelpayoutsToken')}</label>
            <input
              type="password"
              value={travelpayouts.token}
              onChange={e => setTravelpayouts(c => ({ ...c, token: e.target.value }))}
              placeholder={travelpayouts.token_set ? '••••••••' : ''}
              autoComplete="off"
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-slate-400 focus:border-transparent"
            />
            <p className="text-xs text-slate-400 mt-1">
              {travelpayouts.token_from_env ? t('admin.travelpayoutsTokenEnv') : t('admin.travelpayoutsTokenHint')}
            </p>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('admin.travelpayoutsMarker')}</label>
            <input
              type="text"
              inputMode="numeric"
              value={travelpayouts.marker}
              onChange={e => setTravelpayouts(c => ({ ...c, marker: e.target.value }))}
              placeholder="560283"
              className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-slate-400 focus:border-transparent"
            />
            <p className="text-xs text-slate-400 mt-1">{t('admin.travelpayoutsMarkerHint')}</p>
          </div>
          <button
            onClick={async () => {
              setSavingTravelpayouts(true)
              try {
                const payload: Record<string, unknown> = { marker: travelpayouts.marker }
                // Omitted when left blank, so saving the marker alone never wipes a stored token.
                if (travelpayouts.token) payload.token = travelpayouts.token
                await adminApi.updateTravelpayouts(payload)
                setTravelpayouts(c => ({ ...c, token: '', token_set: c.token_set || !!c.token }))
                toast.success(t('admin.travelpayoutsSaved'))
              } catch (err: unknown) {
                toast.error(getApiErrorMessage(err, t('common.error')))
              } finally {
                setSavingTravelpayouts(false)
              }
            }}
            disabled={savingTravelpayouts}
            className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800 disabled:opacity-50"
          >
            {savingTravelpayouts ? t('common.saving') : t('common.save')}
          </button>
        </div>
      </div>
