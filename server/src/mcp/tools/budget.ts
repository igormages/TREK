...
import { generateDailyPlan, clearDailyPlan } from '../../services/budgetPlanService';
...
export function registerBudgetTools(server: McpServer, userId: number, scopes: string[] | null): void {
  ...
  if (W) server.registerTool(
    'generate_daily_budget',
    {
      description:
        "Fill in a trip's day-to-day spending — three meals, two legs of local transport, and the entrance fees for the places already assigned to each day — from the itinerary itself. You pass a RATE PER COUNTRY, not the lines: the server knows which day falls in which country and what is planned on it, and expands that into one expense per meal/leg/day (roughly 900 lines on a six-month trip). Rates are for the whole party, in the trip currency, and default to a table for eating where locals eat; any country you list overrides it field by field. Re-running replaces only what a previous run generated — expenses entered by hand are never touched. Use dry_run first to show the traveller the totals before writing.",
      inputSchema: {
        tripId: z.number().int().positive(),
        rates: z
          .record(
            ...
            z.object({
              ...
              transport_leg: z.coerce.number().min(0).optional().describe('One leg; charged twice a day, there and back'),
              activity: z.coerce.number().min(0).optional().describe('Per place assigned to the day'),
            }),
          )
          .optional()
          .describe('Keyed by ISO 3166-1 alpha-2 country code, plus "*" for anywhere not listed. Omit for the built-in table.'),
        dry_run: z.boolean().optional().describe('Return the totals per country without writing anything.'),
      },
      annotations: TOOL_ANNOTATIONS_WRITE,
    },
    async ({ tripId, rates, dry_run }) => {
      if (isDemoUser(userId)) return demoDenied();
      if (!canAccessTrip(tripId, userId)) return noAccess();
      if (!hasTripPermission('budget_edit', tripId, userId)) return permissionDenied();
      const result = generateDailyPlan(tripId, { rates, dryRun: dry_run });
      if (!result.dry_run) safeBroadcast(tripId, 'budget:bulk-changed', { tripId });
      return ok(result);
    }
  );
  ...
  if (W) server.registerTool(
    ...
    {
      description:
        'Remove every expense generated by generate_daily_budget from a trip. Expenses entered by hand are left alone. This is the undo for a generation run.',
      inputSchema: {
        tripId: z.number().int().positive(),
      },
      annotations: TOOL_ANNOTATIONS_DELETE,
    ...
    async ({ tripId }) => {
      ...
      if (!canAccessTrip(tripId, userId)) return noAccess();
      if (!hasTripPermission('budget_edit', tripId, userId)) return permissionDenied();
      const result = clearDailyPlan(tripId);
      safeBroadcast(tripId, 'budget:bulk-changed', { tripId });
      return ok(result);
    }
  );
